...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // 2. Enforce a two-hour window for this departure slot.
      const now = new Date();
      ...
      const ticket = await tx.ticket.create({
        data: {
          ...
          qrPayload,
          validFrom: now,
          validUntil: new Date(new Date(`${trip.tripDate}T${trip.departureTime}:00+05:30`).getTime() + 30 * 60 * 1000),
          status: 'ACTIVE'
        }
      });
